...
## 1. Introdução
Este estudo analisa a implementação de uma arquitetura de backup "Pull" conectando ambientes de nuvem pública (Provedor Cloud) a infraestruturas privadas (On-Premise). O objetivo é demonstrar como ferramentas de código aberto podem criar soluções de nível corporativo com custo próximo de zero.
...
No modelo **Push**, a Cliente em Nuvem teria as chaves do servidor local. Se a Cliente em Nuvem fosse invadida, o invasor poderia apagar os backups no servidor.
No modelo **Pull** (adotado aqui), o servidor local solicita os dados. A Cliente em Nuvem nunca tem permissão de escrita ou deleção no servidor local, criando uma barreira de segurança física e lógica.
...
A implementação utiliza **SSH Restrito**. Mesmo que a chave SSH da VM Local seja exposta, ela só tem permissão para executar o processo `borg serve` na Cliente em Nuvem, impedindo que alguém use essa chave para navegar nos arquivos ou executar outros comandos no servidor de origem.

## 6. Conclusão
A combinação de Servidor Local e Cliente em Nuvem sob a orquestração do BorgBackup prova ser uma solução robusta para o gerenciamento de desastres, oferecendo segurança criptográfica e eficiência de armazenamento superior a métodos tradicionais de cópia de arquivos.
